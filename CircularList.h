...
DoubleNode<T>* CircularList<T>::find(int index) 
{
   ...
   if (index >= loc_pos)
   {
       ...
       dist_prev= (-1)*((sze-index)+loc_pos);                             //distance using the bridge (prev refs, negative)
   }
   else
   {
       dist_prev=(-1)*(loc_pos-index);                             //distance without the bridge(prev refs, negative)
       dist_next = (sze -loc_pos)+index;                             //distanc with the bridge (next refs, postive)
   }
...
void CircularList<T>::remove(int index) 
{
   ...
   if (index >= 1 && index <= sze) 
   {
      ...
      else
      {
         //use local variables
		loc = after;
		 if(index==sze) // if the index is the last element, it sets the loc_position back to 1.
		 {
			 loc_pos=1;
		 }
